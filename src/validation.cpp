...
const std::string strMessageMagic = "Classic Bitcoin Signed Message:\n";
...
bool static IsCBTCHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.CBTCHeight;
}

bool IsCBTCHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsCBTCHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsCBTCHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsCBTCHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.CBTCHeight;
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight >= consensusParams.CBTCHeight && nHeight < consensusParams.CBTCHeight + consensusParams.CBTCPremineWindow) {
        return COIN * 210000;
    }
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;
    if (fSkipHardforkIBD && chainActive.Tip()->nHeight + 1 >= (int)chainParams.GetConsensus().CBTCHeight)
        return false;
    int64_t target_time = fCBTCBootstrapping ? (int64_t)chainParams.GetConsensus().BitcoinPostforkTime : GetTime();
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsCBTCHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.CBTCHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after CBTC fork.
    if (nHeight >= consensusParams.CBTCHeight && block.nHeight != (uint32_t)nHeight)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.CBTCHeight &&
        nHeight < consensusParams.CBTCHeight + consensusParams.CBTCPremineWindow && false)
    {

// Copyright (c) 2017 The Classic Bitcoin developers
...
#include "test/cbtc_cltv_multisig_data.h"
...
BOOST_FIXTURE_TEST_SUITE(cbtc_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(zcash_header_compatible)
{
    ...
    cbtc_header.nHeight = 10000000;
    // `CBlockHeader.nReserved[5:7]` should be placed in the last 8 bytes of `ZcashBlockHeader.hashReserved`.
    cbtc_header.nReserved[5] = 1234;
    cbtc_header.nReserved[6] = 0;
    cbtc_header.nSolution = std::vector<unsigned char> {
        0x11, 0x22, 0x33
    };
    ...
    BOOST_CHECK_EQUAL(cbtc_header.nHeight, (uint32_t)zcash_header.hashReserved.GetUint64(0));
    BOOST_CHECK_EQUAL(cbtc_header.nReserved[5], (uint32_t)zcash_header.hashReserved.GetUint64(3));
    BOOST_CHECK_EQUAL(cbtc_header.nSolution[2], zcash_header.nSolution[2]);
}

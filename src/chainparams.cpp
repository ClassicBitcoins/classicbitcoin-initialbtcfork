...
public:
    CMainParams() {
        ...
        consensus.CBTCHeight = 516095; // Around 1/4/2018 04:26 UTC
        consensus.CBTCPremineWindow = 10;
        consensus.CBTCPremineEnforceWhitelist = true;
        consensus.BitcoinPostforkBlock = uint256S("000d95ca9692845e40942215c0bb79e35e4a4c14c67fb2ac37507f16c0943f89");
        consensus.BitcoinPostforkTime = 1523519699;
        ...
        //based on https://github.com/classicbitcoins/cbitcoin/issues/
...
consensus.defaultAssumeValid = uint256S("0x0000040608e4707e21bc8efc5c53093279923eca07a462ecddac521790dd0af9"); //528695
        ...
        nDefaultPort = 7123; // different port than Bitcoin
        ...
        vSeeds.emplace_back("dnsseed.bitclassic.info", true);
        vSeeds.emplace_back("dnsseed.classicbitcoin.info", true);

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,28);  // prefix: C
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,75);  // prefix: X
        ...
        checkpointData = (CCheckpointData) {
            {
                ...
                {516000, uint256S("0x00000000000000000043b944fb6352f275d268666a47f598ced0f7b1d58637f5")},
                {528695, uint256S("0x0000040608e4707e21bc8efc5c53093279923eca07a462ecddac521790dd0af9")},
            }
        };
        ...
        vPreminePubkeys = {
        	{ "03358C2C57C93EFE64E1826E8B6CA3416ACA19750A83342289521D7D51BA7D8288", "025CBA8E9209664F0A095E6FAC2E76234672AD3E386623E6F4E1A74396FCCE3DF1", "024C3AFD056DA1812BB3AB741C58527F1265F937CD842273A042498FD8B70E89F2", "0336AFF5AA5F400E30143C6A28E75258B8C0D6AACFCC49B7EBE1A1E8DBC584B487", "0329F8B1437ADE62EA48E62F6CB483895652E6516D07E308C6908C329354250303", "021B0C32A5A4BA8BB2909EE304E38850256D5FC5E10118DBCE82D599EBCAC4CE5D"},
        { "02CBE7A11DAE38DE4641FDE689AD311EEABC3F5A42E6A80CDCB731A68AB12379F1", "03B01B10B578B57A570898B65D013DEF53C8ADCC7B1CB733893BD115466CAC7A6C", "0287F684216591F4112AEAD8A44EB031B86E5F87FA8ECC61F3449704365A575E15", "035F6FD298ED84711AFE3538F906411C8D9AAAB1213D3A57EFF08A4356460E0A24", "03E3AE256D8A2A2FB4BDD5CAACF58CCB7AB7F1B1B82FC2D0AEC047C06A4A3A6718", "0374711D368D8B016C95E57670748622EFE3D9BFE2F17CD3B76CE86384275A7862"},
        };
    }
};
...
public:
    CTestNetParams() {
        ...
        consensus.CBTCHeight = 20161;
         consensus.CBTCPremineWindow = 10;
        consensus.CBTCPremineEnforceWhitelist = false;
        consensus.BitcoinPostforkBlock = uint256S("00378dbe86d3a681dc395d582751c398dc185e4fc3169ab7d35c87c53173cf53");
        //consensus.BitcoinPostforkBlock = uint256S("00000000ef93c2d9bac8da61ff11a699a5b815f77a194c567c5de0dbdf0bf28b");
        consensus.BitcoinPostforkTime = 1523275217;
        ...
        // The best chain should have at least this much work.
        consensus.nMinimumChainWork = uint256S("0x0000000000000000000000000000000000000000000000000001755495464b30");
        ...
        vSeeds.emplace_back("test-dnsseed.bitclassic.info", true);
        vSeeds.emplace_back("test-dnsseed.classicbitcoin.info", true);
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    ...
    assert((uint32_t)consensus.CBTCHeight <= height &&
           height < (uint32_t)(consensus.CBTCHeight + consensus.CBTCPremineWindow));
    if (!consensus.CBTCPremineEnforceWhitelist) {
        return true;
    }
    int block = height - consensus.CBTCHeight;
    int num_unlocked = consensus.CBTCPremineWindow * 60 / 100;  // 40% unlocked.
    int num_locked = consensus.CBTCPremineWindow - num_unlocked;  // 60% time-locked.
    ...
    if (block < num_unlocked) {
        ...
        int lock_time = consensus.CBTCHeight + stage_lock_time * (1 + stage);

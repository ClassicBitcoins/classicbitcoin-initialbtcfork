...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (pblock->nHeight < (uint32_t)params.GetConsensus().CBTCHeight) {
            // Solve sha256d.
...
// NOTE: Unlike wallet RPC (which use CBTC values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in CBTC)\n"
